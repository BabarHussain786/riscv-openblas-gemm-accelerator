...
merge_int8() {
  merged="${OUT_DIR}/int8_square_${M}_runs${RUNS}.log"
  {
    ...
    echo "Generated: $(date)"
    echo
    for f in "${OUT_DIR}"/igemm_kernel_8x8_*_lmul*_unroll*_AI_*_square_results.log \
             "${OUT_DIR}"/igemm_kernel_8x8_*_lmul*_unroll*_GP_*_square_results.log; do
      [ -f "${f}" ] || continue
...
run_family "INT8_iGEMM_LMUL1" "${INT8_ROOT}" "igemm_kernel_8x8_*_lmul1_unroll*"
run_family "INT8_iGEMM_LMUL2" "${INT8_ROOT}" "igemm_kernel_8x8_*_lmul2_unroll*"
run_family "INT8_iGEMM_LMUL4" "${INT8_ROOT}" "igemm_kernel_8x8_*_lmul4_unroll*"
run_family "INT8_iGEMM_LMUL8" "${INT8_ROOT}" "igemm_kernel_8x8_*_lmul8_unroll*"
run_family "INT8_iGEMM_MF2" "${INT8_MF2_ROOT}" "igemm_kernel_8x8_*_lmulmf2_unroll*"
run_family "INT8_iGEMM_MF4" "${INT8_MF4_ROOT}" "igemm_kernel_8x8_*_lmulmf4_unroll*"
if [ "${INCLUDE_MF8}" = "1" ]; then
  run_family "INT8_iGEMM_MF8" "${INT8_MF8_ROOT}" "igemm_kernel_8x8_*_lmulmf8_unroll*"
else
