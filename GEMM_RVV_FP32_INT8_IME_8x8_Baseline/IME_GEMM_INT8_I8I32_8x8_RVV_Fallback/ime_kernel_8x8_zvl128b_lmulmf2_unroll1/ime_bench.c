...
int ime_kernel_8x8_zvl128b_lmulmf2_unroll1(
    ...
    OFLOAT alpha, const IFLOAT *A, const IFLOAT *B, OFLOAT *C, BLASLONG ldc
);
...
int main(int argc, char** argv)
{
    ...
    int rc = ime_kernel_8x8_zvl128b_lmulmf2_unroll1(M, N, K, 1, A, B, C, M);
    if (rc != 0) {
        printf("KERNEL_RETURN=%d\n", rc);
        fprintf(stderr, "kernel failed, rc=%d\n", rc);
        free(A);
        free(B);
        free(C);
        return rc;
