...
run_ime() {
  mapfile -t dirs < <(find "${IME_ROOT}" -maxdepth 1 -type d -name "ime_kernel_8x4_*_unroll*" | sort)
  if [ "${#dirs[@]}" -eq 0 ]; then
  ...
  for d in "${dirs[@]}"; do
    ...
    (
      ...
      for target in "${RUN_TARGETS[@]}"; do
        ...
        if [ "${label}" = "GP" ]; then
          {
            echo "NOTE: IME on GP cpuset uses the matched INT8 RVV fallback path when IME hardware is unavailable."
          } >> "${log_file}"
          echo "[INFO] ${kernel} on GP (${cpuset}) should use the matched INT8 RVV fallback path." | tee -a "${LIVE_LOG}"
        fi
...
merge_ime() {
  merged="${OUT_DIR}/ime_square_${M}_runs${RUNS}.log"
  {
    ...
    echo "Generated: $(date)"
    echo
    for f in "${OUT_DIR}"/ime_kernel_8x4_*_unroll*_AI_*_square_results.log \
             "${OUT_DIR}"/ime_kernel_8x4_*_unroll*_GP_*_square_results.log; do
      [ -f "${f}" ] || continue
      echo "=================================================="
