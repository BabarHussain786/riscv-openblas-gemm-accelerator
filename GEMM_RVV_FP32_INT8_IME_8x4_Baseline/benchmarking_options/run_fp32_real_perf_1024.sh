...
run_FP32() {
  mapfile -t dirs < <(find "${FP32_ROOT}" -maxdepth 1 -type d -name "sgemm_kernel_8x4_*_lmul*_unroll*" | sort)
  if [ "${#dirs[@]}" -eq 0 ]; then
...
merge_FP32() {
  merged="${OUT_DIR}/FP32_square_${M}_runs${RUNS}.log"
  {
    ...
    echo "Generated: $(date)"
    echo
    for f in "${OUT_DIR}"/sgemm_kernel_8x4_*_lmul*_unroll*_AI_*_square_results.log \
             "${OUT_DIR}"/sgemm_kernel_8x4_*_lmul*_unroll*_GP_*_square_results.log; do
      [ -f "${f}" ] || continue
      echo "=================================================="
