...
Tile: 8x4
Variant: igemm_kernel_8x4_zvl256b_lmulmf2_unroll2
LMUL label: mf2
...
static inline void vec_block_8xN(BLASLONG cols, BLASLONG K,
                                 ...
                                 OFLOAT *Cblk, BLASLONG ldc)
{
    const size_t gvl = __riscv_vsetvl_e8mf2(4);
    vint8mf2_t a8_0;
    vint8mf2_t a8_1;
    vint16m1_t a16_0;
    vint16m1_t a16_1;
    vint32m2_t r00 = __riscv_vmv_v_x_i32m2(0, gvl);
    vint32m2_t r01 = __riscv_vmv_v_x_i32m2(0, gvl);
    vint32m2_t r02 = __riscv_vmv_v_x_i32m2(0, gvl);
    vint32m2_t r03 = __riscv_vmv_v_x_i32m2(0, gvl);
    vint32m2_t r10 = __riscv_vmv_v_x_i32m2(0, gvl);
    vint32m2_t r11 = __riscv_vmv_v_x_i32m2(0, gvl);
    vint32m2_t r12 = __riscv_vmv_v_x_i32m2(0, gvl);
    vint32m2_t r13 = __riscv_vmv_v_x_i32m2(0, gvl);
...
#pragma GCC unroll 2
    for (BLASLONG k = 0; k < K; ++k) {
        a8_0 = __riscv_vle8_v_i8mf2(&Ablk[ai], gvl);
        a8_1 = __riscv_vle8_v_i8mf2(&Ablk[ai + gvl], gvl);
        a16_0 = __riscv_vsext_vf2_i16m1(a8_0, gvl);
        a16_1 = __riscv_vsext_vf2_i16m1(a8_1, gvl);
        ai += 8;

        if (cols > 0) {
            const int16_t b = (int16_t)Bblk[bi + 0];
            r00 = __riscv_vwmacc_vx_i32m2(r00, b, a16_0, gvl);
            r10 = __riscv_vwmacc_vx_i32m2(r10, b, a16_1, gvl);
        }
        if (cols > 1) {
            const int16_t b = (int16_t)Bblk[bi + 1];
            r01 = __riscv_vwmacc_vx_i32m2(r01, b, a16_0, gvl);
            r11 = __riscv_vwmacc_vx_i32m2(r11, b, a16_1, gvl);
        }
        if (cols > 2) {
            const int16_t b = (int16_t)Bblk[bi + 2];
            r02 = __riscv_vwmacc_vx_i32m2(r02, b, a16_0, gvl);
            r12 = __riscv_vwmacc_vx_i32m2(r12, b, a16_1, gvl);
        }
        if (cols > 3) {
            const int16_t b = (int16_t)Bblk[bi + 3];
            r03 = __riscv_vwmacc_vx_i32m2(r03, b, a16_0, gvl);
            r13 = __riscv_vwmacc_vx_i32m2(r13, b, a16_1, gvl);
        }
        bi += cols;
    }
    ...
    if (cols > 0) {
        __riscv_vse32_v_i32m2(tmp0, r00, gvl);
        __riscv_vse32_v_i32m2(tmp1, r10, gvl);
    ...
    if (cols > 1) {
        __riscv_vse32_v_i32m2(tmp0, r01, gvl);
        __riscv_vse32_v_i32m2(tmp1, r11, gvl);
    ...
    if (cols > 2) {
        __riscv_vse32_v_i32m2(tmp0, r02, gvl);
        __riscv_vse32_v_i32m2(tmp1, r12, gvl);
    ...
    if (cols > 3) {
        __riscv_vse32_v_i32m2(tmp0, r03, gvl);
        __riscv_vse32_v_i32m2(tmp1, r13, gvl);
                                 ...
                                 OFLOAT *Cblk, BLASLONG ldc)
{
    ...
    vint8mf2_t a8;
    vint16m1_t a16;
    vint32m2_t r0 = __riscv_vmv_v_x_i32m2(0, gvl);
    vint32m2_t r1 = __riscv_vmv_v_x_i32m2(0, gvl);
    vint32m2_t r2 = __riscv_vmv_v_x_i32m2(0, gvl);
    vint32m2_t r3 = __riscv_vmv_v_x_i32m2(0, gvl);
...
#pragma GCC unroll 2
    for (BLASLONG k = 0; k < K; ++k) {
        a8 = __riscv_vle8_v_i8mf2(&Ablk[ai], gvl);
        a16 = __riscv_vsext_vf2_i16m1(a8, gvl);
        ai += 4;

        if (cols > 0) {
            const int16_t b = (int16_t)Bblk[bi + 0];
            r0 = __riscv_vwmacc_vx_i32m2(r0, b, a16, gvl);
        }
        if (cols > 1) {
            const int16_t b = (int16_t)Bblk[bi + 1];
            r1 = __riscv_vwmacc_vx_i32m2(r1, b, a16, gvl);
        }
        if (cols > 2) {
            const int16_t b = (int16_t)Bblk[bi + 2];
            r2 = __riscv_vwmacc_vx_i32m2(r2, b, a16, gvl);
        }
        if (cols > 3) {
            const int16_t b = (int16_t)Bblk[bi + 3];
            r3 = __riscv_vwmacc_vx_i32m2(r3, b, a16, gvl);
        }
        bi += cols;
    }
    ...
    OFLOAT tmp[4];
    if (cols > 0) {
        __riscv_vse32_v_i32m2(tmp, r0, gvl);
        OFLOAT *c_col = &Cblk[0 * ldc];
        for (BLASLONG m = 0; m < 4; ++m) {
    ...
    if (cols > 1) {
        __riscv_vse32_v_i32m2(tmp, r1, gvl);
        OFLOAT *c_col = &Cblk[1 * ldc];
        for (BLASLONG m = 0; m < 4; ++m) {
            c_col[m] = add_scaled_wrap_i32(c_col[m], alpha, tmp[m]);
        }
    ...
    if (cols > 2) {
        __riscv_vse32_v_i32m2(tmp, r2, gvl);
        OFLOAT *c_col = &Cblk[2 * ldc];
        for (BLASLONG m = 0; m < 4; ++m) {
            c_col[m] = add_scaled_wrap_i32(c_col[m], alpha, tmp[m]);
        }
    ...
    if (cols > 3) {
        __riscv_vse32_v_i32m2(tmp, r3, gvl);
        OFLOAT *c_col = &Cblk[3 * ldc];
        for (BLASLONG m = 0; m < 4; ++m) {
            c_col[m] = add_scaled_wrap_i32(c_col[m], alpha, tmp[m]);
        }
