...
Tile: 8x4
Variant: igemm_kernel_8x4_zvl256b_lmul4_unroll4
LMUL label: 4
LMUL compute path: true_i8m4_scalar_i32_accumulation
Unroll: 4
...
static inline void vec_block_8xN(BLASLONG cols, BLASLONG K,
                                 ...
                                 OFLOAT *Cblk, BLASLONG ldc)
{
    const size_t gvl = __riscv_vsetvl_e8m4(8);
    IFLOAT a_lane[8];
    int64_t acc[32] = {0};
    BLASLONG ai = 0;
    BLASLONG bi = 0;
...
#pragma GCC unroll 4
    for (BLASLONG k = 0; k < K; ++k) {
        vint8m4_t a8 = __riscv_vle8_v_i8m4(&Ablk[ai], gvl);
        __riscv_vse8_v_i8m4(a_lane, a8, gvl);
        ai += 8;
...
static inline void vec_block_4xN(BLASLONG cols, BLASLONG K,
                                 ...
                                 OFLOAT *Cblk, BLASLONG ldc)
{
    const size_t gvl = __riscv_vsetvl_e8m4(4);
    IFLOAT a_lane[4];
    int64_t acc[16] = {0};
    BLASLONG ai = 0;
    BLASLONG bi = 0;
