...
FP64_ROOT="${BASE_DIR}/FP64_Double_dgemm_kernel_8x4"
...
run_fp64() {
  mapfile -t dirs < <(find "${FP64_ROOT}" -maxdepth 1 -type d -name "dgemm_kernel_8x4_zvl128b_lmul*_unroll*" | sort)
  if [ "${#dirs[@]}" -eq 0 ]; then
...
merge_fp64() {
  merged="${OUT_DIR}/fp64_square_${M}_runs${RUNS}.log"
  {
    ...
    echo "Generated: $(date)"
    echo
    for f in "${OUT_DIR}"/dgemm_kernel_8x4_zvl128b_lmul*_unroll*_AI_*_square_results.log \
             "${OUT_DIR}"/dgemm_kernel_8x4_zvl128b_lmul*_unroll*_GP_*_square_results.log; do
      [ -f "${f}" ] || continue
      echo "=================================================="
