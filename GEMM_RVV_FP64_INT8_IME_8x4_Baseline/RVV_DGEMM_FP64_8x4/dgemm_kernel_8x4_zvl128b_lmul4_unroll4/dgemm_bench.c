...
/* Explicitly declare the unroll 4 kernel */
int dgemm_kernel_8x4_zvl128b_lmul4_unroll4(
...
int main(int argc, char** argv)
{
    ...
    // Allocate 64-byte aligned memory for FP64 arrays
    ...
    // Call the specific unroll 4 kernel
    int rc =     dgemm_kernel_8x4_zvl128b_lmul4_unroll4(M, N, K, 1.0, A, B, C, M);
    double t1 = now_sec();
    ...
    double time = t1 - t0;
    double gflops = (2.0 * (double)M * N * K) / (time * 1e9);
