...
/*
RVV INT KERNEL
Variant: igemm_kernel_8x4_zvl128b_lmul8_unroll1_i8i32
Settings:
 input_LMUL=8 (m8)
 M=8
 N=4
 ...
 unroll_factor=1
 Kernel Name: igemm_kernel_8x4_zvl128b_lmul8_unroll1_i8i32

 Legality note:
 A true vector-widened m8 INT8 -> INT32 path would exceed the RVV maximum
 accumulator LMUL. This implementation keeps the variant honest by using true
 e8,m8 vector loads and scalar INT32 accumulation.
*/
#include <stddef.h>
#include <stdint.h>
...
#define INPUT_VSETVL(n) __riscv_vsetvl_e8m8(n)
typedef vint8m8_t input_i8_t;
#define INPUT_VLE8(p, vl) __riscv_vle8_v_i8m8(p, vl)
#define INPUT_STORE(p, v, vl) __riscv_vse8_v_i8m8(p, v, vl)
...
int igemm_kernel_8x4_zvl128b_lmul8_unroll1_i8i32(
...
#pragma GCC unroll 1
                for (BLASLONG k = 0; k < K; ++k) {
                    ...
                    INPUT_STORE(a_tmp, a8, gvl);

                    for (size_t lane = 0; lane < gvl; ++lane) {
                        size_t out_idx = (size_t)base + lane;
                        GEMM_I32 a = (GEMM_I32)a_tmp[lane];
                        out0[out_idx] += a * (GEMM_I32)b0;
                        out1[out_idx] += a * (GEMM_I32)b1;
                        out2[out_idx] += a * (GEMM_I32)b2;
                        out3[out_idx] += a * (GEMM_I32)b3;
                    }
                }
