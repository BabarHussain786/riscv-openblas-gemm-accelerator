...
merge_int8() {
  merged="${OUT_DIR}/int8_square_${M}_runs${RUNS}.log"
  {
    ...
    echo "Generated: $(date)"
    echo
    for f in "${OUT_DIR}"/igemm_kernel_8x8_zvl256b_lmul*_unroll*_AI_*_square_results.log \
             "${OUT_DIR}"/igemm_kernel_8x8_zvl256b_lmul*_unroll*_GP_*_square_results.log; do
      [ -f "${f}" ] || continue
      grep -q '^Status: OK$' "${f}" || continue
      grep -q '^GOPS:' "${f}" || continue
      echo "=================================================="
...
echo "INCLUDE_MF8=${INCLUDE_MF8}, KEEP_OLD_LOGS=${KEEP_OLD_LOGS}" | tee -a "${LIVE_LOG}"
if [ "${HAVE_TASKSET}" -eq 1 ]; then
...
run_family "INT8_iGEMM_LMUL1" "${INT8_ROOT}" "igemm_kernel_8x8_zvl256b_lmul1_unroll*"
run_family "INT8_iGEMM_LMUL2" "${INT8_ROOT}" "igemm_kernel_8x8_zvl256b_lmul2_unroll*"
run_family "INT8_iGEMM_LMUL4" "${INT8_ROOT}" "igemm_kernel_8x8_zvl256b_lmul4_unroll*"
run_family "INT8_iGEMM_LMUL8" "${INT8_ROOT}" "igemm_kernel_8x8_zvl256b_lmul8_unroll*"
run_family "INT8_iGEMM_MF2" "${INT8_MF2_ROOT}" "igemm_kernel_8x8_zvl256b_lmulmf2_unroll*"
run_family "INT8_iGEMM_MF4" "${INT8_MF4_ROOT}" "igemm_kernel_8x8_zvl256b_lmulmf4_unroll*"
if [ "${INCLUDE_MF8}" = "1" ]; then
  run_family "INT8_iGEMM_MF8" "${INT8_MF8_ROOT}" "igemm_kernel_8x8_zvl256b_lmulmf8_unroll*"
else
