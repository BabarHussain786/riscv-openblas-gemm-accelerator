...
run_ime() {
  mapfile -t dirs < <(find "${IME_ROOT}" -maxdepth 1 -type d -name "ime_kernel_8x8_zvl256b_*_unroll*" | sort)
  if [ "${#dirs[@]}" -eq 0 ]; then
...
merge_ime() {
  merged="${OUT_DIR}/ime_square_${M}_runs${RUNS}.log"
  {
    ...
    echo "Generated: $(date)"
    echo
    for f in "${OUT_DIR}"/ime_kernel_8x8_zvl256b_*_unroll*_AI_*_square_results.log \
             "${OUT_DIR}"/ime_kernel_8x8_zvl256b_*_unroll*_GP_*_square_results.log; do
      [ -f "${f}" ] || continue
