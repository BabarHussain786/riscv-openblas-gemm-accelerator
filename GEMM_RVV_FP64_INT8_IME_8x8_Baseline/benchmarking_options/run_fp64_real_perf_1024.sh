...
LIVE_LOG="${OUT_DIR}/fp64_live_${TS}.log"
...
run_fp64() {
  mapfile -t dirs < <(find "${FP64_ROOT}" -maxdepth 1 -type d -name "dgemm_kernel_8x8_zvl256b_lmul*_unroll*" | sort)
  if [ "${#dirs[@]}" -eq 0 ]; then
    echo "[ERROR] No FP64 kernel directories found." | tee -a "${LIVE_LOG}"
    exit 1
  fi
...
merge_fp64() {
  merged="${OUT_DIR}/fp64_square_${M}_runs${RUNS}.log"
  {
    echo "FP64 merged results"
    echo "Generated: $(date)"
    echo
    for f in "${OUT_DIR}"/dgemm_kernel_8x8_zvl256b_lmul*_unroll*_AI_*_square_results.log \
             "${OUT_DIR}"/dgemm_kernel_8x8_zvl256b_lmul*_unroll*_GP_*_square_results.log; do
      [ -f "${f}" ] || continue
...
build_run_targets
cleanup_old_logs
echo "FP64 real-performance run" | tee "${LIVE_LOG}"
